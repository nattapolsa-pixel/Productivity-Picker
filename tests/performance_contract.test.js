...
assert(html.includes('app.js?v=20260916-remove-weighted-kpi-v100'), 'HTML must cache-bust the latest release');
...
assert(app.includes('function getTargetForZoneOrType') && (app.includes('const getTargetForType = getTargetForZoneOrType;') || app.includes('getTargetForZoneOrType(')),
  'Zone target lookup must support getTargetForZoneOrType and getTargetForType alias');
assert(app.includes('function getTypePickForZone'), 'Zone type pick lookup helper must be available');
...
assert(app.includes("aggregate('PTT', dfrom, dto, shiftF)") && app.includes("aggregate('BPS', dfrom, dto, shiftF)") &&
  app.includes("aggregate('ALL', dfrom, dto, shiftF)"),
  'Sheet analysis must reconcile PTT, BPS and the combined active-hour calculation');
